	...
	render: function() {
		return (
			<div style={this.props.active ? TagButtonStyleActive : TagButtonStyle} onClick={this.props.tagOnClick}>
				{this.props.tag.content.length > 0 ? this.props.tag.content : "全部"} ({this.props.tag.amount})
			</div>
		);
	}
});
...
var TagBox = React.createClass({
	...
	componentDidMount: function() {
		$.get("/api/" + idStr + "/tags/my", function(result) {
			tags = [];
			for (key in result) {
				tags.push({
					content: key,
...
var ExprBox = React.createClass({
	...
	reloadExprs: function(tag) {
		$.get("/api/" + idStr + "/exprs/my?tag=" + tag, function(result) {
			if (this.isMounted()) {
				this.setState({
					exprs: result,
				});
